...
function del(id, user, request) {
    ...
    subscriberTable.where({
        ...
        success: function (items) {
            if (items.length === 0) {
                request.respond(statusCodes.NOT_FOUND, { error: 'No matching item found to delete.' });
            } else {
                if (items[0].ownerId != user.userId) {
                    request.respond(statusCodes.BAD_REQUEST, { error: "Cannot delete someone else's item." });
                } else {
                    request.execute();
                }
            }
