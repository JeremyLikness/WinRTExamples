...
namespace SafeNotes.Data
{
    ...
    public class ViewModel : BindableBase, ICommand 
    {
        ...
        {
            var descriptionText = string.IsNullOrWhiteSpace(this.currentNote.Description)
                ? "[Note is empty]"
                : this.currentNote.Description;

            var descriptionLength = descriptionText.Length > 20 ? 20 : descriptionText.Length;
            var endDescription = descriptionText.Length > 20 ? descriptionText.Length - 20 : 0;

            TileTemplateType.TileWide310x150Text09.GetTile()
                            .WithNotifications()
                            .AddText(this.currentNote.Title)
                            .AddText(this.currentNote.Description)
                            .WithTile(
                                TileTemplateType.TileSquare150x150Text02.GetTile()
                                                .AddText(this.currentNote.Title)
                                                .AddText(this.currentNote.Description))
                            .WithTile(
                                TileTemplateType.TileSquare310x310Text09.GetTile()
                                                .AddText(this.currentNote.Title)
                                                .AddText(string.Format("Created: {0}", this.currentNote.DateCreated))
                                                .AddText(string.Format("Modified: {0}", this.currentNote.DateModified))
                                                .AddText(string.Format("{0}...", descriptionText.Substring(0, descriptionLength)))
                                                .AddText(string.Format("...{0}", descriptionText.Substring(endDescription))))
                                                .Set();
        }
